...
Каждый отсек имеет свое здоровье *(и предназначение?)*. При попадании в него здоровье изменяется:  
`Health = Health - (Damage * ShieldMultiplier)`  
где `Health` - текущее здоровье отсека,   
`Damage` - величина урона орудия,   
...
Если уровень здоровья падает до нуля, то отсек "умирает". Бой происходит до момента, когда все отсеки будут выведены из строя.
