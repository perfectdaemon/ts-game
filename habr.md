...
* TypeScript позволяет отойти от строгости, если требуется реализовать что-то быстро и/или грязно.
...
* Я не смог настроить дебаг. Не хватает дзена. Знаю, что нужны source map, но подружить TypeScript, Webpack и sourcemap-ы у меня пока не вышло.
* Небольшой шаг влево/вправо — и вы совсем одни. Возникла проблема с Angular и Typescript? Вам помогут первые две ссылки на stack overflow. Возникло желание подружиться с Typescript отдельно и возник странный баг/вопрос? Готовьтесь к тому, что вы вполне можете оказаться в одиночестве. Конечно, я, как человек ранее писавшй на очень популярном (на самом деле нет) стеке FreePascal + OpenGL, к этому привычен. Но в случае зрелого языка, вроде C++, всегда найдется индус, который уже решал похожую проблему. А затем и китаец, который решил ее с помощью контроллера от Guitar Hero и Arduino.
